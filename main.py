...
parser.add_argument('--plots', type=str, default='temp',
                    help='path to save tensorboard files')
...
writer = SummaryWriter('runs/'+args.plots)
...
def train():
    ...
    for batch_idx, start_idx in enumerate(range(0, N, args.batch_size)):
        ...
        optimizer.zero_grad()
        recon_batch, lat_var, mu, logvar = model(data)
        true_samples = torch.randn(lat_var.size()).to(device)
        
        loss = criterion(recon_batch, data, lat_var, true_samples, mu, logvar, anneal, device)
        loss.backward()
...
def evaluate(data_tr, data_te):
    ...
    with torch.no_grad():
        for start_idx in range(0, e_N, args.batch_size):
            ...
            recon_batch, lat_var, mu, logvar = model(data_tensor)
            true_samples = torch.randn(lat_var.size()).to(device)

            loss = criterion(recon_batch, data_tensor, lat_var, true_samples,mu, logvar, anneal, device)
